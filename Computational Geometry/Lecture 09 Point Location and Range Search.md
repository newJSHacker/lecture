...
- Demo: `Computational Geometry/code/09-naive-intersect.html` (local, no CDN). Serve the folder if ES modules fail.
- Backup: board first — a point set, a query rectangle, brute force vs pruned boxes.
- Parked strip: `Lecture 9 | Point location and range search | Invariant: predicates before constructions; degeneracy is the course`
- Quiz from last lecture (except Lecture 1 / midterm / presentations).

## Board at the end (they photograph this)

```
a point set, a query rectangle, brute force vs pruned boxes
Slabs through vertices.
kd-tree splits on 8 points.
A query rectangle vs a node cell: disjoint / contained / overlap.
BVH of three triangles.
```
