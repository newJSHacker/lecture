...
- Demo: `Computational Geometry/code/02-on-segment.html` (local, no CDN). Serve the folder if ES modules fail.
- Backup: board first — the five objects (point, segment, ray, line, polygon).
- Parked strip: `Lecture 2 | Geometric primitives | Invariant: predicates before constructions; degeneracy is the course`
- Quiz from last lecture (except Lecture 1 / midterm / presentations).

## Board at the end (they photograph this)

```
the five objects (point, segment, ray, line, polygon)
The four intersection types.
Ray casting with a vertex hit, then the same figure with the half-open rule marked.
AABB of two disjoint crossing-looking segments that do not meet.
```
