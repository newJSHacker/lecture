...
**Week 14 of 15** · Interactive Web Development  
**Meeting:** studio (not a content lecture)  
**Kernel:** interactive page  
...
- Clock visible.
- Demo only if a volunteer asks for a blocked kernel: `Interactive Web/code/09-gsap.html`.
- Parked strip: `Lecture 14 | Goal: freeze and review | Invariant: time is rAF; input is events; draw is a function`
...
**Say:** Desk review: loop and input first.
...
| Min | Beat | Plant / fix |
| ---: | --- | --- |
| 0–10 | Start the kernel: interactive page | Plant the first common mistake. |
| 10–30 | Desk review: loop and input first. | Fix on the board; they copy. |
...
See [[Interactive Web/exercises/Week 14]].

## Notes from the outline

**1. Ideas.** instrument, tiny game, data sketch, HUD mock configurator 2D.
**2. Cuts.** Drop GSAP. Keep loop + input.
**3. Report.** loop, input, perf note.
